...
- Replaced the former derived, curated, and native-mirroring collection contracts without importing their definitions. Existing Jellyfin BoxSets are left unchanged.
...
- Added durable account-scoped collection configuration, playback preferences, custom posters, crash-safe local-account deletion, rebuildable collection snapshots, exact secondary-provider identity mapping, and automatic profile refresh. Legacy account settings import once, and interrupted local-account deletion resumes safely.
...
- Added Companion administration for MDBList and TMDB, plus the authenticated `ratings-v1` Desktop boundary. Data Protection encrypts administrator secrets, and API responses never return them. Capability and status responses require authentication and redact sensitive values. MDBList uses bounded stable-id batches, a shared persistent cache, duplicate-request suppression, and restart-safe quota or `Retry-After` backoff. Validated TMDB credentials also power the collection experience.
- Added Releases with agenda and month views, release-type filters, stale-data warnings, Today navigation, local Play or Open actions, and film requests. Home shows an upcoming releases shelf, and Companion refreshes its complete monitored calendar daily. Without Companion, Releases shows Jellyfin's metadata-only Upcoming feed.
...
- Replaced the top header with a collapsible sidebar for search, Home, Movies, Series, and Favorites. The signed-in user and server stay at the bottom with library sync and sign-out actions. Home keeps the sidebar open; other routes use an icon rail that expands on hover.
...
- Expanded Discover with Seerr's upcoming film and series feeds, illustrated genre browsing for both media types, sorting by popularity, rating, or date, minimum-score filters, trending controls, and a local-library filter. Search loads later result pages while scrolling. Cards show their TMDB score and a clear Request or Open details action. Desktop exposes only the allowlisted Companion contract.
...
- Added complete cast filmographies to cast search. Jellyfin stores only the leading cast members of each title, so a person can be in the library without their name in the stored cast list. Cast pages now take the person's full TMDB credits and verify unmatched ones against the server: proven titles appear under "More on your Jellyfin server", and Discover never offers a request for a title the server already has. The local catalog maps candidates to Jellyfin item ids, which are verified in batches of up to 100. A normal cast page therefore needs one live ownership request regardless of how many extra titles it finds, and any failed proof degrades to the previous behavior instead of hiding or mislabeling results.
...
- Renovate now waits one day after a dependency release before proposing an update.
...
- Bumped the library schema to 14. There is no pre-1.0 migration. MediaFlick deletes older databases and rebuilds the catalog, while preserving the signed-in session and device id needed for the sync.
...
- Seerr discovery, requests, status, and cancellation now run only through the authenticated MediaFlick Companion contract. Desktop still matches results to the local library, and Companion writes are not retried after an uncertain response.
...
- Simplified Settings. It now opens on Player instead of an overview page. The navigation shows page names without repeating their descriptions, card rating sources appear only under Appearance, and one read-only Companion page reports server integrations while provider setup remains in Jellyfin administration.
...
- Fixed requested collection cards keeping stale Seerr availability. Missing titles now refresh their request status, while local collection titles use the standard library card and actions.
- Fixed account configuration being coupled to the replaceable library cache. Appearance and Letterboxd profiles now live in an atomic, versioned `accounts.json` keyed by Jellyfin server and user IDs; logout and catalog rebuilds retain them, account switches isolate them, and device settings live in `settings.json`.
...
- Fixed the closed streaming-quality selector on details rendering transparently over backdrop art, which made its text hard to read on light scenes. It now uses an opaque card background with the artwork drop shadow. The Details, Media, and Cast sections also use opaque panels and contrast shadows on their labels, so they stay readable over both light and dark backdrops.
- Fixed curated MDBList definitions resolving to zero items after the list request selected MDBList's incompatible ID-only response. Curated sync now requests the ranked movie and series payload, reports a provider-shape error instead of caching a false empty list, and keeps curated settings intact when an administrator replaces a protected provider key.
- Fixed the Collections page returning only part of the library's TMDB collections. Movie detail lookups no longer discard the shared Companion mapping cache, collection summaries ignore mappings outside the current library, and Desktop loads every bounded mapping batch before rendering the page.
...
- Fixed `just run` and `just run-non-debug` reopening an older Windows process through the single-instance gate. Development recipes now stop the staged app before rebuilding and launching it.
